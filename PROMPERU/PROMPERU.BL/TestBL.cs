    ...
    public class TestBL
    {
                ...
                if (testModel.Elements != null && testModel.Elements.Count > 0)
                {
                    foreach (var e in testModel.Elements)
                    {
                        if (e.Type == "question")
                        {
                            ...
                            // Si la pregunta es computable, insertar curso relacionado
                            if (e.IsComputable == true && e.Course != null)
                            {
                                var preguntaCurso = new PreguntaCursoBE
                                {
                                    Preg_ID = preguntaID,
                                    Curs_ID = e.Course.Value
                                };

                                await _preguntaCursoDA.InsertarPreguntaCursoAsync(preguntaCurso, usuario, ip);
                            }
        ...
        public async Task<TestModelDto> ObtenerTestPorIdAsync(int Id)
        {
            try
            {
                ...
                var inscripcions = inscripcionsTask.Result;
                ...
                // Obtener portada
                var portadaTes = portadaTestTask.Result;
                ...
                var preguntaTest = preguntaTestTask.Result;
                var respuestaCursoTest = preguntaCursoTestTask.Result;
                var respuestaTest = respuestaTestTask.Result;

                // Asegurarse de que test.Elements está inicializado
                if (test.Elements == null)
                {
                    test.Elements = new List<Element>();
                }
                ...
                test.Elements.AddRange(
                    preguntaTest
                        ...
                        .Select(p => new Element
                        {
                            ...
                            // Si no hay cursos, asignar null
                            Course = respuestaCursoTest
                                .Where(r => r.Preg_ID == p.ID)
                                .Select(r => new Course
                                {
                                    Value = r.Curs_ID,
                                    Label = r.Curs_NombreCurso
                                })
                                .FirstOrDefault(),
                ...
                var contenidoTest = contenidoTestTask.Result;
                var formularioTest = formularioTestTask.Result;
