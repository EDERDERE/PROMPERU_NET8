...
function loadEditarCurso() {
    ...
    $('#saveEditTitulo').click(function () {
        ...
        if (data.titulo && data.nombreBotonTitulo && data.urlIconBoton) {
            handleAjaxRequest('/Curso/ActualizarCurso', data, 'El Curso se ha actualizado exitosamente.', 'No se pudo actualizar el Curso.');
        } else {
...
function loadEliminarCurso() {
    ...
    const eliminarCurso = (id) => {
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    showAlert('success', '¡Eliminado!', 'El curso ha sido eliminado con éxito.', () => {
                        location.reload(); // Recargar la página o actualizar el contenido dinámicamente
                    });
                } else {
                    showAlert('error', 'Error', 'No se pudo eliminar el curso. Inténtalo de nuevo.');
                }
            },
            error: function () {
                showAlert('error', 'Error', 'Hubo un error en el servidor al intentar eliminar el curso.');
            }
        });
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            showAlert('success', '¡Actualizado!', 'El curso se ha actualizado exitosamente.', () => {
                                location.reload(); // Recargar la página o actualizar el contenido
                            });
                        } else {
                            showAlert('error', 'Error', 'No se pudo actualizar el curso. Inténtelo nuevamente.');
                        }
                    },
                    error: function (xhr, status, error) {
                        showAlert('error', 'Error', 'Hubo un error al intentar actualizar el curso.');
                    }
                });
            }
        });
