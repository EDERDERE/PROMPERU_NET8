  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        response.perfilEmpresarials.forEach((perfilEmpresarial) => {
          console.log("lista PerfilEmpresarial", perfilEmpresarial);
          var sliderCard = `
          <div class="card col-12 col-md-12 shadow-lg border-0 p-4 mb-3" data-id="${perfilEmpresarial.pemp_ID}">
            <div class="d-flex justify-content-between align-items-start mb-3">
              <h5 class="card-number mb-0"></h5>
              <div class="d-flex gap-2">
                <button class="btn btn-link text-danger p-0" 
                  id="btn-delete-${perfilEmpresarial.pemp_ID}"
                      data-id=${perfilEmpresarial.pemp_ID} >
                ...
                data-id="${perfilEmpresarial.pemp_ID}"
                data-nombre="${perfilEmpresarial.pemp_Nombre}"
                data-description="${perfilEmpresarial.pemp_Descripcion}"
                data-image-url="${perfilEmpresarial.pemp_UrlImagen}">
             ...
             <div class="mb-3">
              <label for="description-${perfilEmpresarial.pemp_ID}" class="form-label fw-semibold">Nombre</label>
              <input id="description-${perfilEmpresarial.pemp_ID}" class="form-control" rows="3" placeholder="${perfilEmpresarial.pemp_Nombre}" disabled/>
            </div>
            <div class="mb-3">
              <label for="description-${perfilEmpresarial.pemp_ID}" class="form-label fw-semibold">Descripción</label>
              <textarea id="description-${perfilEmpresarial.pemp_ID}" class="form-control" rows="3" placeholder="${perfilEmpresarial.pemp_Descripcion}" disabled></textarea>
            </div>
            <div>
              <label for="image-url-${perfilEmpresarial.pemp_ID}" class="form-label fw-semibold">URL de la imagen</label>
              <input type="text" id="image-url-${perfilEmpresarial.pemp_ID}" class="form-control" value="${perfilEmpresarial.pemp_UrlImagen}" disabled>
            </div>
          </div>`;
...
function loadEditarPerfilEmpresarial() {
  $("#editSliderModal").on("show.bs.modal", function (event) {
    ...
    modal.find("#editNombre").val(nombrePerfil); // Llenar el campo de la URL de la imagen
  });
  $("#saveEditSlider").click(function () {
    ...
    var id = $("#editId").val();
    var nombrePerfil = $("#editNombre").val();

    if (description && imageUrl) {
      $.ajax({
        type: "POST",
        url: "/PerfilEmpresarial/ActualizarPerfilEmpresarial", // URL del controlador para editar el slider
        data: {
          nombre: nombrePerfil,
          id: id,
          description: description,
