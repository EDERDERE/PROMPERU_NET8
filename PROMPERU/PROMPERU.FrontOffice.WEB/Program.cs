...
// Configuraci�n de Serilog desde appsettings.json
builder.Host.UseSerilog((context, services, configuration) =>
    configuration
        .ReadFrom.Configuration(context.Configuration) // Cargar configuraci�n de appsettings.json
        .WriteTo.Console() // Escribir tambi�n en consola (opcional)
);
...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =>
    {
        options.LoginPath = "/Test/Index"; // Redirige a Login si no est� autenticado
        options.LogoutPath = "/Test/CerrarSesion"; // Ruta para cerrar sesi�n
...
app.UseStaticFiles(); // Para cargar archivos CSS o im�genes
app.UseRouting();
