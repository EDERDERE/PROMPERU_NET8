$(document).ready(function () {
  layout.loadListarLogos();
  layout.loadListarMenus();
  layout.loadListarFooters();
});
  ...
  loadListarMenus: function () {
    $.ajax({
      ...
      success: function ({ success, menus, message }) {
        console.log("Respuesta de menus:", menus);
        ...
        if (success) {
          if (menus.length > 0) {
            ...
            $menuHome.html("<p>No hay información de menus disponibles.</p>");
          }
        } else {
          Swal.fire({
            icon: "error",
