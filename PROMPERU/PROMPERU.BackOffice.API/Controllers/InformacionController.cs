    ...
    public class InformacionController : Controller
    {
        ...
        public async Task<IActionResult> InsertarInformacion(InformacionDto informacionDto)
        {
            try
            {
                ...
                await _informacionBL.InsertarInformacionAsync(informacion, usuario, ip); // Llamada asincr�nica
        ...
        public async Task<IActionResult> ActualizarInformacion(InformacionDto informacionDto, int id)
        {
            try
            {
                ...
                await _informacionBL.ActualizarInformacionAsync(informacion, usuario, ip, id); // Llamada asincr�nica
                return RedirectToAction("ListarInformacions");
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> EliminarInformacion(int id)
        {
            try
            {
                var usuario = HttpContext.Session.GetString("Usuario");// Usuario autenticado
                string ip = HttpContext.Connection.RemoteIpAddress?.ToString();
                await _informacionBL.EliminarInformacionAsync(usuario, ip, id); // Llamada asincr�nica
                return RedirectToAction("ListarInformacions");
            }
            catch (Exception ex)
            {
        ...
        //    try
        //    {
        //        var banner = await _bannerBL.ObtenerBannerAsync(bannID); // Llamada asincr�nica
        //        return View(banner); // Aseg�rate de tener una vista para mostrar un banner
        //    }
        //    catch (Exception ex)
        //    {
        //        ViewBag.Error = ex.Message;
